...
export function DashboardSidebar({ activeLabel = "Dashboard" }: DashboardSidebarProps) {
  return (
    <div className="h-full min-h-[calc(100vh-44px)] p-3">
      <p className="rounded bg-cyan-950/50 px-3 py-2 text-sm font-semibold text-cyan-100">{activeLabel}</p>
      <div className="mt-4 space-y-4">
        {menuSections.map((section) => (
          <div key={section.label}>
            <p className="mb-1 text-xs font-semibold uppercase tracking-wider text-zinc-500">▾ {section.label}</p>
            <ul className="space-y-0.5">
              {section.links.map((link) => (
                <li key={link}>
                  <button
                    type="button"
                    className="block w-full rounded px-2 py-1 text-left text-sm text-zinc-300 transition hover:bg-zinc-700/40 hover:text-white"
                  >
                    {link}
                  </button>
                </li>
